...
<?php foreach($abr->getBillReceivers() as $index => $row){ 
    ...
    <p>70% der Heizkosten sind <?=$gas->PreisHeizung70ProzentE?>. Geteilt durch die Gesamtsumme der Messwerte (<?=$hkv->totalMeteredConsumption($gas->Abrechnungsjahr)?>) ergibt das einen Preis pro Messwert von <?=$hkv->preisProMesswert?> €.</p>
    <p>Letzterer mal <strong><?=$mC?></strong> gemessene Werte macht <strong><?=$gas->euro($hkv->preisProMesswert * $mC)?></strong> verbrauchsmäßig zugeordnete Heizkosten.</p>
